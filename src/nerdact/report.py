...
</style></head><body><main><div class="eyebrow">NERDACT · MODEL COMPARISON</div><h1>A larger model changes the tradeoffs</h1><p><a href="index.html">Baseline transcript report</a> · <a href="roberta-large.html">RoBERTa transcript report</a></p>
<div class="card"><p>Both models saw the same 10 fictional call transcripts and the same first {limit} examples from the CoNLL-2003 <strong>{html.escape(split)}</strong> split. Domain metrics require exact character boundaries and labels. Leakage and over-redaction measure character coverage.</p></div>
<table><thead><tr><th rowspan="2">Model</th><th colspan="5">Synthetic call transcripts</th><th colspan="3">CoNLL {html.escape(split)} ({limit})</th></tr><tr><th>Precision</th><th>Recall</th><th>F1</th><th>Leakage</th><th>Over-redaction</th><th>Precision</th><th>Recall</th><th>F1</th></tr></thead><tbody>{"".join(table_rows)}</tbody></table>
<div class="card"><h2>What this comparison can—and cannot—say</h2><p>The larger RoBERTa checkpoint reports stronger validation performance and was separately tested by its author on informal email/chat text. It also has roughly three times as many parameters, so quality comes with higher download, memory, and inference costs.</p><p>A perfect score on ten designed examples is encouraging, not proof of safety. This corpus is intentionally small and synthetic. The next meaningful step is a larger, independently labeled transcript set with harder negatives and entity types that match the intended redaction policy.</p></div>
...
</style></head><body><header><div class="eyebrow">NERDACT · CHARACTER-SPAN EVALUATION</div><h1>Domain report</h1><p>Model <code>{model}</code><br>Revision <code>{revision}</code></p><p><a href="benchmark.html">Compare the baseline and larger model →</a></p><div class="warning"><strong>Teaching demo, not a privacy system.</strong> This news-trained model only recognizes PERSON, ORGANIZATION, LOCATION, and MISCELLANEOUS. It does not detect addresses, phone numbers, email, or general PII.</div></header><main>
