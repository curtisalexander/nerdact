...
<h2>Reproduce</h2><pre><code>{command}</code></pre><p><small>Models are revision-pinned. Generated reports contain the original input text and must be protected when using non-synthetic data.</small></p></main></body></html>"""
    html_path.parent.mkdir(parents=True, exist_ok=True)
...
</style></head><body><header><div class="eyebrow">NERDACT · CHARACTER-SPAN EVALUATION</div><h1>Domain report</h1><p>Model <code>{model}</code><br>Revision <code>{revision}</code></p><nav>{_site_nav()}</nav><div class="warning"><strong>Teaching demo, not a privacy system.</strong> This news-trained model only recognizes PERSON, ORGANIZATION, LOCATION, and MISCELLANEOUS. It does not detect addresses, phone numbers, email, or general PII.</div></header><main>
...
</style></head><body><main><div class="eyebrow">NERDACT · PHASE 3</div><h1>Labels became inference inputs</h1><nav>{_site_nav()}</nav><p>GLiNER <code>{html.escape(artifact["model"])}</code> at revision <code>{html.escape(artifact["revision"])}</code></p>
...
<h2>Threshold and wording sweep</h2><table><thead><tr><th>Label wording</th><th>Threshold</th><th>Precision</th><th>Recall</th><th>F1</th><th>Leakage</th><th>Over-redaction</th></tr></thead><tbody>{threshold_rows}</tbody></table><div class="card warning"><strong>Not an independent calibration set.</strong> These curves describe the same small designed corpus used for evaluation. Selecting its best threshold and reporting that score as generalization would be optimistic; a deployment threshold needs separate, representative calibration data.</div>
...
</style></head><body><main><div class="eyebrow">NERDACT · PHASE 4</div><h1>Hybrid detection reduces—but does not eliminate—PII leakage</h1><nav>{_site_nav()}</nav><p>Contextual checkpoint <code>{html.escape(artifact["model"])}</code><br>Revision <code>{html.escape(artifact["revision"])}</code> · Apache-2.0</p>
